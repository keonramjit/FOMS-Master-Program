...
export const FlightPlanning: React.FC<FlightPlanningProps> = ({
  ...
  // Load data from props when date changes or flights load
  useEffect(() => {
  ...
  // Helper to format Decimal Time to H:MM
  ...
  const handleRouteChange = (id: string, val: string) => {
    ...
    // Attempt to look up route logic
    const routeDef = routes.find(r => r.code === code);
    
    setLocalFlights(prev => prev.map(f => {
        if (f.id === id) {
             const updates: any = { route: code };
             
             // If route is found in DB, auto-populate times based on Aircraft Type
             if (routeDef) {
                const type = f.aircraftType || 'C208B';
                let flightTime = routeDef.flightTime || 0;
                let commTime = routeDef.commercialTimeC208B || 0;

                if (type === '1900D') {
                  flightTime = routeDef.flightTime1900D || flightTime;
                  commTime = routeDef.commercialTime1900D || commTime;
                } else if (type === 'C208EX') {
                  flightTime = routeDef.flightTimeC208EX || flightTime;
                  commTime = routeDef.commercialTimeC208EX || commTime;
                } else {
                  flightTime = routeDef.flightTimeC208B || flightTime;
                }

                updates.flightTime = flightTime;
                
                if(commTime) {
                    updates.commercialTime = decimalToHm(commTime);
                }
             }
             return { ...f, ...updates };
        }
    ...
    <div className="flex flex-col h-full bg-slate-50 relative">
      ...
      <div className="flex-1 overflow-auto p-4 lg:px-6 lg:py-4 custom-scrollbar">
        <div className="min-w-[1300px] flex flex-col pb-10">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {groupedFlights.map((flight) => {
                                        ...
                                        <div className={cellWrapperClass}>
                                            <input 
                                                list="location-options" // UPDATED to match datalist ID
            ...
            {/* DATALIST for Locations - Using locations prop now! */}
